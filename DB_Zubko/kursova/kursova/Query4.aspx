<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="Query4.aspx.cs" Inherits="kursova.Query4" %>
...
<body>
    ...
    <div>
        ...
        <asp:SqlDataSource ID="SqlDataSource1" runat="server" ConnectionString="<%$ ConnectionStrings:KursovaDBConnectionString1 %>" DeleteCommand="DELETE FROM [Worker] WHERE [WorkerID] = @WorkerID" InsertCommand="INSERT INTO [Worker] ([FirstName], [SecondName], [DateBirth], [Experience], [PositionID], [TransportID]) VALUES (@FirstName, @SecondName, @DateBirth, @Experience, @PositionID, @TransportID)" SelectCommand="SELECT * FROM [Worker] WHERE YEAR(DateBirth)&lt;1980" UpdateCommand="UPDATE [Worker] SET [FirstName] = @FirstName, [SecondName] = @SecondName, [DateBirth] = @DateBirth, [Experience] = @Experience, [PositionID] = @PositionID, [TransportID] = @TransportID WHERE [WorkerID] = @WorkerID">
            <DeleteParameters>
